document.addEventListener('DOMContentLoaded', function(){
  ...
  function validateField(input){
    ...
    if(name === 'pesel'){
      if(!/^[0-9]{11}$/.test(v)) return 'PESEL musi zawierać dokładnie 11 cyfr';
    }
    if(name === 'first_name'){
    ...
    if(name === 'phone_number'){
      if(!/^[0-9+\-\s]{9,}$/.test(v)) return 'Numer telefonu nie jest poprawny';
    }
    if(name === 'email'){
      ...
      if(local.startsWith('.') || local.endsWith('.') || domain.startsWith('.') || domain.endsWith('.')) return 'E-mail nie może zaczynać ani kończyć się kropką';
      // no consecutive dots anywhere in local or domain
      if(local.includes('..') || domain.includes('..')) return 'E-mail nie może zawierać podwójnych kropek';
      // domain must contain at least one dot (e.g. example.com)
      if(domain.indexOf('.') === -1) return 'Domena e-mail musi zawierać kropkę (np. example.com)';
    }
    if(name === 'birth_date'){
  ...
  form.addEventListener('submit', function(e){
    ...
    if(!resident || !resident.checked){
      ...
      if(!el){
        el = document.createElement('div'); el.className = 'field-error';
    ...
    if(!note){
      note = document.createElement('div'); note.className = 'form-success';
