...
function validateNumber(
  input,
  opts = { min: null, max: null, required: true },
) {
  clearError(input);
  ...
  if (v !== null) {
    if (opts.min !== null && v < opts.min) {
      setError(input, `Minimalna wartość to ${opts.min}`);
      return false;
    }
    if (opts.max !== null && v > opts.max) {
      setError(input, `Maksymalna wartość to ${opts.max}`);
      return false;
    }
  }
  return true;
...
function validateDate(input, opts = { required: false }) {
  ...
  const v = input.value;
  if (opts.required && !v) {
    setError(input, "To pole jest wymagane");
    return false;
  }
  if (v) {
    const d = new Date(v);
    if (Number.isNaN(d.getTime())) {
      setError(input, "Nieprawidłowa data");
      return false;
    }
  }
  return true;
...
function validateNotes(input, opts = { maxLen: 256 }) {
  ...
  if (v.length > opts.maxLen) {
    setError(input, `Maksymalna długość to ${opts.maxLen} znaków`);
    return false;
  }
  return true;
}
...
function validateForm(form) {
  ...
  if (idEstate && (!idEstate.value || idEstate.value === "")) {
    setError(idEstate, "Wybierz nieruchomość");
    ok = false;
  }
  ...
  if (idEstate && idEstate.value === "new" && newEstateFields) {
    ...
    if (osiedle && !osiedle.value) {
      setError(osiedle, "Wybierz osiedle");
      ok = false;
    }
    if (
      type &&
      (!type.value || (type.value !== "mieszkanie" && type.value !== "dom"))
    ) {
      setError(type, "Wybierz typ nieruchomości");
      ok = false;
    }
    ...
    if (type && type.value === "mieszkanie") {
      if (attachment) {
        ...
        if (!files || files.length === 0) {
          setError(attachment, "Dołącz zaświadczenie (pdf/jpg/png)");
          ok = false;
        } else {
          const f = files[0];
          ...
          if (f.size > maxBytes) {
            setError(attachment, "Plik nie może być większy niż 10 MB");
            ok = false;
          }
          if (f.type && allowed.indexOf(f.type) === -1) {
            setError(
              attachment,
              "Nieprawidłowy typ pliku (dozwolone: pdf, jpg, png)",
            );
            ok = false;
          }
          ...
          if (!f.type) {
            ...
            if (!name.match(/\.pdf$|\.jpe?g$|\.png$/)) {
              setError(attachment, "Nieprawidłowy plik (rozszerzenie)");
              ok = false;
            }
          }
        }
    ...
    if (postal && postal.value) {
      if (!/^[0-9]{2}-[0-9]{3}$/.test(postal.value)) {
        setError(postal, "Kod pocztowy powinien mieć format XX-XXX");
        ok = false;
      }
    } else {
      setError(postal, "Wpisz kod pocztowy");
      ok = false;
    }
    if (street && (!street.value || street.value.trim().length < 2)) {
      setError(street, "Wpisz nazwę ulicy");
      ok = false;
    }
    if (building && (!building.value || building.value.trim().length < 1)) {
      setError(building, "Wpisz numer budynku");
      ok = false;
    }
  ...
  if (arrival) {
    if (!validateDate(arrival, { required: false })) ok = false;
  }
  if (depart) {
    if (!validateDate(depart, { required: false })) ok = false;
  }
...
window.addEventListener("load", function () {
  ...
  if (idEstateSelect && newEstateBlock) {
    function toggleNewEstate() {
      if (idEstateSelect.value === "new") {
        ...
        // reset inputs, textareas and selects to their default values
        qsa("input, textarea, select", newEstateBlock).forEach((i) => {
  ...
  // Or if building is an apartment
  const estatesDiv = qs("#hidden_building_type", form);
  // parse estates JSON (if present) into a map of id -> estate_type
  let estateTypes = {};
  if (estatesDiv && estatesDiv.dataset && estatesDiv.dataset.estates) {
    try {
      const arr = JSON.parse(estatesDiv.dataset.estates || "[]");
      if (Array.isArray(arr)) {
        arr.forEach((e) => {
          // ensure id_estate exists and estate_type may be 'apartment'|'house'
          if (e && e.id_estate != null) estateTypes[String(e.id_estate)] = e.estate_type;
        });
      }
    } catch (err) {
      // ignore parse errors
      console.warn("Failed to parse estates JSON", err);
    }
  }

  // if current type is apartment, show attachment field
  const newType = qs("#new_est_type", form);
  const attachmentField = qs("#file-attachment-container", form);
  const attachmentInput = qs("#new_est_attachment", form);

  function shouldShowAttachment() {
    // If user is adding a new estate and selected type is 'mieszkanie'
    if (newType && newType.value === "mieszkanie") return true;
    // If user selected an existing estate and its type is 'apartment'
    if (idEstateSelect && idEstateSelect.value && idEstateSelect.value !== "new") {
      const t = estateTypes[idEstateSelect.value];
      if (t === "apartment") return true;
    }
    return false;
  }
  ...
  if (attachmentField) {
    ...
    // Listen to both the new type select and the estate select
    if (newType) newType.addEventListener("change", toggleAttachment);
    if (idEstateSelect) idEstateSelect.addEventListener("change", toggleAttachment);
    // run once on load
    toggleAttachment();
  }
  ...
  form.addEventListener("submit", function (e) {
    ...
    if (!success) {
      ...
      success.textContent = "Wniosek przygotowany.";
